...
import csv   # for read csv file
import os    # for navigating in file system 
import numpy as np  # for creating array 
from PIL import Image,ImageTk  # for performing image operation 
import pandas as pd  # for doing operation on csv file or data
import datetime
import time
import random
...
def manually_fill():
    ...
    def err_screen_for_subject():
        ...
        ec.title('Warning!!')
        ec.configure(background='#778899')
    ...
    def fill_attendance():
        ...
        if subb=='':
            ...
            ENR = tk.Label(MFW, text="Enter Enrollment", width=15, height=2, fg="Cornsilk", bg="#708090",
                           font=('times', 15, ' bold '))
            ENR.place(x=30, y=100)

            STU_NAME = tk.Label(MFW, text="Enter Student name", width=15, height=2, fg="Cornsilk", bg="#708090",
                                font=('times', 15, ' bold '))
            STU_NAME.place(x=30, y=200)

            global ENR_ENTRY
            ENR_ENTRY = tk.Entry(MFW, width=20,validate='key', bg="#708090", fg="Cornsilk", font=('times', 23, ' bold '))
            ...
            STUDENT_ENTRY = tk.Entry(MFW, width=20, bg="#708090", fg="Cornsilk", font=('times', 23, ' bold '))
            ...
            def create_csv():
                ...
                csv_name='F:/JIT/Attendace_management_system-master/Attendance/Manually_Attendance/'+DB_table_name+'.csv'
            ...
            def attf():
                import subprocess
                subprocess.Popen(r"F:\\JIT\\Attendace_management_system-master\\Attendance\\Manually_Attendance\\")

            attf = tk.Button(MFW,  text="Check Sheets",command=attf,fg="white"  ,bg="lawn green"  ,width=12  ,height=1 ,activebackground = "Red" ,font=('times', 14, ' bold '))
            attf.place(x=730, y=410)

            MFW.mainloop()


    SUB = tk.Label(sb, text="Enter Subject", width=15, height=2, fg="Cornsilk", bg="#708090", font=('times', 15, ' bold '))
    SUB.place(x=30, y=100)

    global SUB_ENTRY

    SUB_ENTRY = tk.Entry(sb, width=20, bg="#708090", fg="Cornsilk", font=('times', 23, ' bold '))
    SUB_ENTRY.place(x=250, y=105)

    fill_manual_attendance = tk.Button(sb, text="Fill Attendance",command=fill_attendance, fg="black", bg="#778899", width=20, height=2,
                       activebackground="Red", font=('times', 15, ' bold '))
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                root.configure(background='snow')
                cs = 'F:/JIT/Attendace_management_system-master/' + fileName
    ...
    def Attf():
        import subprocess
        subprocess.Popen(r'explorer /select,"F:\\JIT\\Attendace_management_system-master\\Attendance\\"')
    ...
    sub = tk.Label(windo, text="Enter Subject", width=15, height=2, fg="Cornsilk", bg="#708090", font=('times', 15, ' bold '))
    sub.place(x=30, y=100)

    tx = tk.Entry(windo, width=20, bg="white", fg="black", font=('times', 23, ' bold '))
    tx.place(x=250, y=105)

    fill_a = tk.Button(windo, text="Fill Attendance", fg="black",command=Fillattendances, bg="#778899", width=20, height=2,
                       activebackground="Red", font=('times', 15, ' bold '))
...
def admin_panel():
    ...
    def log_in():
        ...
        if username == 'yash' :
            if password == '1234':
                ...
                cs = 'F://JIT//Attendace_management_system-master//StudentDetails//StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
    ...
    un = tk.Label(win, text="Enter username", width=15, height=2, fg="Cornsilk", bg="#708090",
                   font=('times', 15, ' bold '))
    un.place(x=30, y=50)

    pw = tk.Label(win, text="Enter password", width=15, height=2, fg="Cornsilk", bg="#708090",
                  font=('times', 15, ' bold '))
    pw.place(x=30, y=150)
    ...
    un_entr = tk.Entry(win, width=20, bg="#708090", fg="Cornsilk", font=('times', 23, ' bold '))
    un_entr.place(x=290, y=55)

    def c11():
        pw_entr.delete(first=0, last=22)

    pw_entr = tk.Entry(win, width=20,show="*", bg="#708090", fg="Cornsilk", font=('times', 23, ' bold '))
...
def training():
    ...
    try:
        ...
        Notification.configure(text=l, bg="SpringGreen3", width=40, font=('times', 18, 'bold'))
...
lbl = tk.Label(window, text="Enter Enrollment", width=20, height=2, fg="Cornsilk", bg="#708090", font=('times', 15, ' bold '))
...
txt = tk.Entry(window, validate="key", width=20, bg="#708090", fg="Cornsilk", font=('times', 25, ' bold '))
txt['validatecommand'] = (txt.register(testVal),'%P','%d')
txt.place(x=550, y=210)

lbl2 = tk.Label(window, text="Enter Name", width=20, fg="Cornsilk", bg="#708090", height=2, font=('times', 15, ' bold '))
lbl2.place(x=200, y=300)

txt2 = tk.Entry(window, width=20, bg="#708090", fg="Cornsilk", font=('times', 25, ' bold '))
txt2.place(x=550, y=310)

clearButton = tk.Button(window, text="Clear",command=clear,fg="black"  ,bg="#778899"  ,width=10  ,height=1 ,activebackground = "Red" ,font=('times', 15, ' bold '))
clearButton.place(x=950, y=210)

clearButton1 = tk.Button(window, text="Clear",command=clear1,fg="black"  ,bg="#778899"  ,width=10 ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
...
quit = tk.Button(window, text="Quit", command=on_closing  ,fg="Cornsilk"  ,bg="#708090"  ,width=10  ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
quit.place(x=1150, y=610)
